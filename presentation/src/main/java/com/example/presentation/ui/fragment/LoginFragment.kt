...
import com.example.data.network.authentication.FirebaseAuthenticator
    ...
    private fun setupView() {
        with(binding) {
            btnLogin.setOnClickListener {
                val isLoggedIn = com.example.data.network.authentication.FirebaseAuthenticator.provideLoginService().login(tietEmail.text.toString(), tietPassword.text.toString())
                if (isLoggedIn)
